        ...
        match self.page_type {
            PageType::INTERNAL | PageType::LEAF => {
                if i >= self.item_count() {
                    ...
                    K::decode(&self.buf[(self.keys_pos + i * K::bin_size())..])
                        .map(|t| t.0)
                        .ok()
                }
            }
        ...
        match self.page_type {
            PageType::LEAF => {
                if i >= self.item_count() {
                    ...
                    V::decode(&self.buf[(self.values_pos + i * V::bin_size())..])
                        .map(|t| t.0)
                        .ok()
                }
            }
        ...
        match self.page_type {
            PageType::INTERNAL => {
                if i >= self.item_count() + 1 {
                    ...
                    u32::decode(&self.buf[(self.ptrs_pos + i * PTR_SIZE)..])
                        .map(|t| t.0)
                        .ok()
                }
            }
                ...
                match pos {
                    ...
                    Pos::Left => {
                        ...
                        unsafe {
                            ...
                            std::ptr::copy(
                                key_ptr.add(i * K::bin_size()),
                                key_ptr.add((i + 1) * K::bin_size()),
                                (old_item_count - i) * K::bin_size(),
                            );
                            std::ptr::copy(
                                value_ptr.add(i * V::bin_size()),
                                value_ptr.add((i + 1) * V::bin_size()),
                                (old_item_count - i) * V::bin_size(),
                            );
                        }
                    ...
                    Pos::Right => {
                        ...
                        unsafe {
                            ...
                            std::ptr::copy(
                                key_ptr.add((i + 1) * K::bin_size()),
                                key_ptr.add((i + 2) * K::bin_size()),
                                (old_item_count - i - 1) * K::bin_size(),
                            );
                            std::ptr::copy(
                                value_ptr.add((i + 1) * V::bin_size()),
                                value_ptr.add((i + 2) * V::bin_size()),
                                (old_item_count - i - 1) * V::bin_size(),
                            );
                        }
                ...
                match pos {
                    ...
                    Pos::Left => {
                        ...
                        unsafe {
                            ...
                            std::ptr::copy(
                                ptr_ptr.add((i + 1) * PTR_SIZE),
                                ptr_ptr.add((i + 2) * PTR_SIZE),
                                (old_item_count - i) * PTR_SIZE,
                            );
                        }
                        // for j in (i..old_item_count).rev() {
                        //     self.set_key_at(j + 1, &self.key_at(j).unwrap())?;
                    ...
                    Pos::Right => {
                        ...
                        unsafe {
                            ...
                            std::ptr::copy(
                                ptr_ptr.add((i + 2) * PTR_SIZE),
                                ptr_ptr.add((i + 3) * PTR_SIZE),
                                (old_item_count - i - 1) * PTR_SIZE,
                            );
                        }
                        // for j in ((i + 1)..old_item_count).rev() {
                        //     self.set_key_at(j + 1, &self.key_at(j).unwrap())?;
            ...
            PageType::LEAF => {
                ...
                for i in 0..self.item_count() {
                    f.write_fmt(format_args!(
                        "#{} {:?}: {:?}\n",
                        i,
                        self.key_at(i).unwrap(),
                        self.value_at(i).unwrap()
                    ))?;
                }
            }
            PageType::INTERNAL => {
